...
function buildMainFloor(floor: Floor) {
  ...
  if (floor.id === 'EG') {
    for (const wallId of ['stair-east', 'stair-east-south']) {
      const opening = floor.walls.find(wall => wall.id === wallId)!.openings[0]
    ...
    for (const item of floor.furniture.filter(item => ['sofa', 'sofa-chaise', 'coffee'].includes(item.id))) item.z -= .35
    ...
    counter.z = stair.z + .65; counter.depth = 4.55 - counter.z
    floor.furniture.find(item => item.id === 'kitchen-sink')!.z = 3.5
    floor.rooms.find(room => room.id === 'living')!.parts.push(recessRoom)
    floor.furniture.push(furniture('stair-cabinet', 'cabinet', recessRoom.x, recessRoom.z, recessRoom.width, recessRoom.depth, 2.4, Math.PI))
    return
  }
  if (floor.id === 'KG') {
  ...
  if (floor.id === 'OG') {
    const roomDivider = 4.6, eastRoomStart = roomDivider + interiorWallThickness, bathEnd = 3.45, hallEnd = 6.73, southStart = hallEnd + interiorWallThickness, readingEnd = 7.65
    hall.parts[0].depth = hallEnd - stair.z
    floor.walls.push(wall('north-divider', 'z', roomDivider, house.north, bathEnd - house.north), wall('bath-south', 'x', stairEast, stairWallStart, childRoomWest - stairEast), wall('bath-east-return', 'x', eastDividerWest, bathEnd, eastRoomStart - eastDividerWest), wall('east-divider', 'z', eastDividerWest, stair.z, hallEnd - stair.z, interiorWallThickness, [{ ...door('bath', .05), hinge: 'end' }, door('child-north', 1.65)]), wall('children-divider', 'x', childRoomWest, hallEnd, house.east - childRoomWest), wall('store-north', 'x', stairEast, hallEnd, childRoomWest - stairEast, interiorWallThickness, [door('child-south', .17)]), wall('reading-east', 'z', stairEast, southStart, readingEnd - southStart), wall('reading-south', 'x', house.west, readingEnd, hallWest - house.west))
    ...
    floor.rooms = [
      room('bath', 'Familienbad', [rect(house.west, house.north, roomDivider - house.west, stairWallStart - house.north), rect(childRoomWest, stairWallStart, roomDivider - childRoomWest, bathEnd - stairWallStart)], colors.wet, '9,3 m² Nordbad: Wanne längs an der Westwand, Dusche, WC und Waschplatz im Norden. Zugang über die östliche Eingangsnische. Leitungsführung ungeprüft.', [2.85, 1.8]),
      room('child-north', 'Kind Nordost', [rect(eastRoomStart, house.north, house.east - eastRoomStart, bathEnd + interiorWallThickness - house.north), rect(childRoomWest, bathEnd + interiorWallThickness, house.east - childRoomWest, hallEnd - bathEnd - interiorWallThickness)], colors.child, '15,7 m² mit breiter Hauptfläche und nördlicher Arbeitsnische. Schrank an der Südwand, freier Weg vom Eingang zum Schreibtisch.', [4.8, 5.7]),
      room('child-south', 'Kind Süd', [rect(hallWest, southStart, house.east - hallWest, house.south - southStart), rect(house.west, readingEnd + interiorWallThickness, hallWest - house.west, house.south - readingEnd - interiorWallThickness)], colors.child, '15,2 m² mit breiter Hauptfläche und südwestlicher Schreibtischnische. Eigener Flurzugang, Ost- und Südfenster.', [4.4, 8]),
  ...
  if (floor.id === 'DG') {
    ...
    const hallStart = stair.z + stair.runWidth
    hall.parts[0] = rect(hallWest, hallStart, stair.arrivalDepth, stair.end - hallStart)
    hall.note = 'Kurzer Verteiler am letzten Treppenaustritt. Der nördliche ehemalige Flur gehört zu Gäste / Arbeit; die Treppennische bleibt zugänglich.'
    floor.walls.find(wall => wall.id === 'stair-east')!.openings = []
    floor.walls.push(wall('knee-north', 'x', .4, heightLine(1.2), (house.east - house.west)), wall('knee-south', 'x', .4, south, (house.east - house.west)), wall('hall-north', 'x', hallWest, hallStart - interiorWallThickness, childRoomWest - hallWest), wall('office-west', 'z', eastDividerWest, hallStart, officeEnd - hallStart, interiorWallThickness, [door('attic-office', .5)]), wall('office-entry', 'x', eastDividerWest, officeEnd, house.east - eastDividerWest), wall('bedroom-west', 'z', eastDividerWest, officeEnd + interiorWallThickness, southRoomStart - officeEnd - interiorWallThickness), wall('parents-entry-south', 'x', stairEast, stair.end, childRoomWest - stairEast, interiorWallThickness, [{ ...door('attic-parents-south', .17), hinge: 'end', swing: 'reverse' }]))
    ...
    floor.rooms.find(room => room.id === 'office')!.parts.push(rect(hallWest, stairWallStart, childRoomWest - hallWest, hallStart - interiorWallThickness - stairWallStart))
    return
  }
}
