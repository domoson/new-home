  ...
  return <div className="app">
    ...
    <div className="workspace">
      <aside className={`sidebar ${roomsOpen ? 'is-open' : ''}`}>
        ...
        <div className="variant-label">Anbieterentwurf · Osthälfte</div>
      ...
      <main className="drawing-area">
        ...
        <footer className="drawing-footer"><span>{exterior ? 'Außenanlagen · Ost + West' : <><b>6,60 × 11,40 m</b> Außenmaß</>}</span><span>{exterior ? `${format(siteArea)} m² Grundstück` : floorId === 'KG' ? 'Nutzkeller' : `${format(living)} m² Wohnfläche*`}</span><span className="footer-extra">{exterior ? 'Rekonstruierte Grenzen' : `Treppenöffnung ${format(area(stairOpeningParts))} m² separat`}</span><button onClick={() => setInfo(true)}>Vorentwurf · ungeprüft <Info size={14} /></button></footer>
      </main>
    </div>
