...
it('bietet reale Masse fuer Moebel, Oeffnungen, Waende, Podeste und Aussenanlagen', () => {
  ...
  expect(objects.findLast(object => contains(object, { x: 4.5, z: 5.6 }))?.label).toBe('Induktionskochfeld')
  expect(objects.findLast(object => contains(object, { x: 6.1, z: 4.9 }))?.label).toBe('Geschirrspüler')
  expect(objects.findLast(object => contains(object, { x: 5.4, z: 2.8 }))?.label).toBe('Siebträgermaschine')
  expect(objects.findLast(object => contains(object, { x: 6.2, z: 3.5 }))?.label).toBe('Cookit / Küchenmaschine')
  expect(objects.findLast(object => contains(object, { x: 5.7, z: 3.05 }))?.label).toBe('Toaster')
