...
export const winderCore = { x: .4, z: 3.1, width: 2.1, depth: 2.2, end: 5.3, tread: .25, risers: 16, runWidth: .9, turnSize: 1.1, arrivalDepth: .95 }
...
export function winderSteps(rise: number): WinderStep[] {
  ...
  const endTread = (core.width - turnSize) / 4
  ...
  for (let index = 0; index < 3; index++) {
    ...
    const width = index === 2 ? 2 * endTread : endTread
    result.push({ id: `south-${index}`, footprint: rectangle(east, core.end - core.runWidth, width, core.runWidth), inner: [[east, core.end - core.runWidth], [east + width, core.end - core.runWidth]], height: (index + 13) * rise / core.risers })
  }
  return result
}
...
export function stairWalkingLine(rise: number): [number, number, number][] {
  ...
  const points: [number, number, number][] = [[core.x + core.width + .6, 0, core.z + halfRun], [center[0], rise * 4 / core.risers, core.z + halfRun]]
  for (let index = 1; index <= 16; index++) {
    const angle = -Math.PI / 2 - index * Math.PI / 16
    points.push([center[0] + radius * Math.cos(angle), rise * (4 + index / 2) / core.risers, center[1] + radius * Math.sin(angle)])
  }
