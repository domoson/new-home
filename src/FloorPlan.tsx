...
export default function FloorPlan({ floor, selected, onSelect, dimensions, furnished, zoom }: { floor: Floor; selected: string; onSelect: (id: string) => void; dimensions: boolean; furnished: boolean; zoom: number }) {
  ...
  return <><div className="measure-tools"><button className={`icon-button ${measuring ? 'on' : ''}`} aria-label="Maßband" title="Maßband: zwei Punkte setzen; Shift für waagerecht/senkrecht" aria-pressed={measuring} onClick={() => { setMeasuring(!measuring); setOrigin(null); setHover(null) }}><Ruler size={19} /></button><button className="icon-button" aria-label="Messungen löschen" title="Messungen löschen" disabled={!lines.length && !origin} onClick={() => { setLines([]); setOrigin(null) }}><Trash2 size={18} /></button>{origin && <button className="icon-button" aria-label="Messung abbrechen" title="Messung abbrechen (Escape)" onClick={() => setOrigin(null)}><X size={18} /></button>}</div><svg xmlns="http://www.w3.org/2000/svg" className="floor-plan" role="img" tabIndex={0} aria-label={`Grundriss ${floor.name}`} viewBox={`${house.width / 2 - viewWidth / 2 - (zoom === 1 ? 0 : pan.x)} ${(floor.id === 'EG' ? 6 : 5) - viewHeight / 2 - (zoom === 1 ? 0 : pan.y)} ${viewWidth} ${viewHeight}`} onPointerDown={event => {
    ...
    {floor.id === 'DG' && <g pointerEvents="none">{[heightLine(2), 10 - heightLine(2)].map(south => <g key={south}><line x1=".4" x2={house.east} y1={south} y2={south} stroke="#608c94" strokeDasharray=".09 .06" strokeWidth=".022" /><text x={house.width - .25} y={south + .04} fontSize=".13" fill="#60818c">2 m</text></g>)}<line x1=".4" x2={house.east} y1="5" y2="5" stroke="#a4aaa1" strokeWidth=".012" strokeDasharray=".25 .05" /></g>}
    ...
    {dimensions && <g stroke="#8a9588" strokeWidth=".014" fill="#667262"><path d={`M0 -.25 V-.85 M${house.width} -.25 V-.85 M0 -.65 H${house.width} M${house.width + .3} 0 H${house.width + .95} M${house.width + .3} 10 H${house.width + .95} M${house.width + .7} 0 V10`} /><path d={`M-.07 -.58 l.14 -.14 M${house.width - .07} -.58 l.14 -.14 M${house.width + .63} .07 l.14 -.14 M${house.width + .63} 10.07 l.14 -.14`} strokeWidth=".025" /><text x={house.width / 2} y="-.79" textAnchor="middle" fontSize=".2" stroke="none">{house.width.toLocaleString('de-DE', { minimumFractionDigits: 2 })} m</text><text x={house.width + 1} y="5" textAnchor="middle" fontSize=".2" stroke="none" transform={`rotate(90 ${house.width + 1} 5)`}>10,00 m</text><path d={`M.4 10.35 V10.7 M${house.east} 10.35 V10.7 M.4 10.55 H${house.east}`} /><text x={house.width / 2} y="10.48" fontSize=".14" textAnchor="middle" stroke="none">{(house.east - house.west).toLocaleString('de-DE', { minimumFractionDigits: 3 })} m lichte Breite</text></g>}
