...
export function createWalker(model: SceneModel, camera: THREE.PerspectiveCamera, spawn: THREE.Vector3) {
  ...
  controller.enableAutostep(.225, .16, false); controller.enableSnapToGround(.3); controller.setMaxSlopeClimbAngle(Math.PI / 4); controller.setMinSlopeSlideAngle(Math.PI / 3)
