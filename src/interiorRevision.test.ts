...
test('Haustuer schwenkt innen und Kind Sued hat einen freien Eingang', () => {
  ...
  const access = rect(3.35, 6.7, .95, 1.5)
...
test('DG-Zugang am Treppenaustritt und Dachoeffnungen auf beiden Dachseiten', () => {
  ...
  expect(area(floor.rooms.find(room => room.id === 'hall')!.parts)).toBeCloseTo(stair.arrivalDepth * stair.depth)
  ...
  expect(area(roofPanels()) + area(roofWindows)).toBeCloseTo(7.85 * 10.5)
