  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      const applySettings = (value: SceneSettings) => {
        ...
        for (const key of ['facade', 'roof', 'frame', 'frameInside'] as const) model.setFinish(key, value[key])
        model.setCladding(value.composition, value.woodTone, value.woodProfile)
        for (const key of ['facade', 'roof', 'frame', 'frameInside'] as const) model.setFinish(key, value.west[key], 'west')
