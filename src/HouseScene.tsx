  ...
  return <><div ref={container} className="scene-container" />
    {!loading && !error && <details className="scene-settings"><summary title="Sonne, Materialien und Öffnungen" aria-label="Szeneneinstellungen"><Settings2 size={18} /><span>Szene</span></summary><div className="settings-body">
      ...
      <fieldset className="finish-settings"><legend>Oberflächen</legend>
        ...
        {(['facade', 'roof', 'roofTrim', 'entryDoor', 'frame', 'frameInside'] as FinishKey[]).map(key => <div className="finish-row" key={key}><span>{{ facade: 'Putzfarbe', roof: 'Dach', roofTrim: 'Dachunterbau', entryDoor: 'Haustür', frame: 'Fenster außen', frameInside: 'Fenster innen' }[key]}</span><div>{finishes[key === 'frameInside' ? 'frame' : key].map(finish => <button key={finish.color} className="color-swatch" style={{ backgroundColor: finish.color }} title={finish.name} aria-label={`${{ facade: 'Fassade', roof: 'Dach', roofTrim: 'Dachunterbau', entryDoor: 'Haustür', frame: 'Fenster außen', frameInside: 'Fenster innen' }[key]} ${finish.name}`} aria-pressed={appearance[key] === finish.color} onClick={() => updateAppearance({ [key]: finish.color })} />)}</div></div>)}
      </fieldset>
