...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, DoorClosed, DoorOpen, House, Lightbulb, LightbulbOff, MousePointer2, RotateCcw, Settings2, Sun } from 'lucide-react'
...
export default function HouseScene({ floorId, mode, furnished, roof, cutWalls, selected, reset, settings, onSettings }: { floorId: FloorId; mode: 'orbit' | 'walk'; furnished: boolean; roof: boolean; cutWalls: boolean; selected: string; reset: number; settings: SceneSettings; onSettings: (settings: SceneSettings) => void }) {
  ...
  const [openings, setOpenings] = useState<{ id: string; label: string; open: boolean; amount: number }[]>([])
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      const updateOpenings = () => { renderer.shadowMap.needsUpdate = true; setOpenings(model.doors.map(door => ({ id: door.id, label: door.label, open: door.open, amount: door.amount }))) }
      updateOpenings(); setOpeningId(model.doors.find(door => door.id.startsWith(floorId) && door.kind === 'window')?.id ?? model.doors[0]?.id ?? '')
      ...
      const toggleOpening = (id?: string) => {
        ...
        updateOpenings(); setOpeningMessage(result ? '' : 'Öffnung zu nah am Körper oder außer Reichweite.'); return result
      }
      const home = () => {
      ...
      const wheel = (event: WheelEvent) => {
        ...
        setOpeningId(opening.id); updateOpenings(); setOpeningMessage(result ? '' : 'Bewegungsweg durch den Körper blockiert.')
      }
      ...
      commands.current = { key(key, pressed) { if (pressed) walker?.keys.add(key); else walker?.keys.delete(key) }, door: toggleOpening, opening(id, amount) { const result = walker ? walker.setOpening(id, amount) : model.setOpening(id, amount); updateOpenings(); setOpeningMessage(result ? '' : 'Bewegungsweg durch den Körper blockiert.') }, lock() { if (pointer.isLocked) pointer.unlock(); else pointer.lock() }, home, facade: facadeView, settings: applySettings }
  ...
  return <><div ref={container} className="scene-container" />
    {!loading && !error && <details className="scene-settings"><summary title="Sonne, Materialien und Öffnungen" aria-label="Szeneneinstellungen"><Settings2 size={18} /><span>Szene</span></summary><div className="settings-body">
      ...
      <label htmlFor="opening-select">Tür / Fenster</label><div className="opening-control"><select id="opening-select" value={openingId} onChange={event => { setOpeningId(event.target.value); setOpeningMessage('') }}>{openings.map(opening => <option key={opening.id} value={opening.id}>{opening.label}</option>)}</select><button className="icon-button" disabled={!currentOpening} aria-label={currentOpening?.open ? 'Ausgewählte Öffnung schließen' : 'Ausgewählte Öffnung öffnen'} title={currentOpening?.open ? 'Schließen' : 'Öffnen'} onClick={() => commands.current?.door(openingId)}>{currentOpening?.open ? <DoorClosed size={19} /> : <DoorOpen size={19} />}</button></div><label htmlFor="opening-amount">Öffnungsgrad</label><input id="opening-amount" type="range" min="0" max="100" step="1" disabled={!currentOpening} value={Math.round((currentOpening?.amount ?? 0) * 100)} onChange={event => commands.current?.opening(openingId, Number(event.target.value) / 100)} /><output className="opening-state">{currentOpening ? `${Math.round(currentOpening.amount * 100)} % ${currentOpening.open ? 'geöffnet' : 'geschlossen'}` : ''}</output>{openingMessage && <p role="status" className="opening-message">{openingMessage}</p>}
    </div></details>}
