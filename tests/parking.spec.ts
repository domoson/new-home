...
test('Carports: Abmessungen, Holztoene, freie Zugaenge und Pfostenkollision', async ({ page }, testInfo) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const placements = siteParking.map(placement => {
      ...
      const roofCorrect = Math.abs(rise - 6 * Math.tan(carportRoof.pitch * Math.PI / 180)) < .00001 && !!structure.getObjectByName('carport-gutter') && !!structure.getObjectByName('carport-downpipe')
