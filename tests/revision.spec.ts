...
test('Hauptentwurf: alle Plaene, weisse Decken, Fenster, Farben und Sonnenstand', async ({ page }, testInfo) => {
  ...
  await page.evaluate(() => window.__house!.door('EG-kitchen-window'))
  ...
  await page.evaluate(() => { window.__house!.door('EG-kitchen-window'); window.__house!.door('DG-office-west-skylight') })
...
test('Innenwände mit Schallschutzstärke und Parkett in den Fluren', async ({ page }) => {
  ...
  const probe = await page.evaluate(async () => {
    ...
    const walls = floorIds.flatMap(id => makeFloor(id).walls.filter(wall => !['west', 'east', 'north', 'south', 'installation'].includes(wall.id)).map(wall => wall.axis === 'x' ? wall.depth : wall.width))
...
test('OG-Badtuer schwenkt nach innen und bleibt von Sanitaerobjekten frei', async ({ page }) => {
  ...
  const probe = await page.evaluate(async () => {
    ...
    const samples = Array.from({ length: 21 }, (_, index) => {
      ...
      return { minZ: bounds.min.z, maxZ: bounds.max.z, collision: fixtures.some(fixture => bounds.intersectsBox(fixture)) }
    })
    model.dispose()
    return { samples, wallZ: wall.z, bathZ: wall.z + wall.depth }
  })
  expect(probe.samples.every(sample => !sample.collision && sample.maxZ <= probe.bathZ)).toBe(true)
  expect(probe.samples.at(-1)!.minZ).toBeLessThan(probe.wallZ - .75)
})
...
test('Osttreppe: alle drei Verbindungen auf und ab sowie offene Tueren', async ({ page }, testInfo) => {
  ...
  await page.evaluate(() => window.__house!.teleport(3.25, 0, 1.65))
  await walkTo(Math.PI / 2, 'x', 1.6, true)
  await walkTo(Math.PI, 'z', 2.2, false)
  await page.evaluate(() => window.__house!.teleport(2.88, -2.7, 3.3))
  await walkTo(0, 'z', 1.7, true)
  await page.evaluate(() => window.__house!.teleport(2.88, -2.7, 4.65))
  await walkTo(-Math.PI / 2, 'x', 5.1, false)
  await page.evaluate(() => window.__house!.teleport(2.88, -2.7, 5.48))
  await walkTo(Math.PI, 'z', 7.4, false)
  await page.evaluate(() => window.__house!.teleport(2.88, 2.95, 4.21))
  await walkTo(-Math.PI / 2, 'x', 5.1, false)
  await page.evaluate(() => window.__house!.teleport(2.88, 2.95, 5.48))
  await walkTo(0, 'z', 1.65, true)
  await page.evaluate(() => window.__house!.teleport(2.88, 2.95, 5.48))
  await walkTo(Math.PI, 'z', 7.3, false)
  await walkTo(Math.PI / 2, 'x', 1.6, true)
  await page.evaluate(() => window.__house!.teleport(2.88, 2.95, 5.83))
  await walkTo(-Math.PI / 2, 'x', 3.9, false)
  await walkTo(Math.PI, 'z', 7.4, false)
  await page.evaluate(() => window.__house!.teleport(2.88, 5.9, 5.48))
  await walkTo(0, 'z', 3.5, true)
  await walkTo(-Math.PI / 2, 'x', 3.8, false)
  await walkTo(0, 'z', 2.75, true)
  expect((await page.evaluate(() => window.__house!.position())).y).toBeGreaterThan(6.7)
  await page.evaluate(() => window.__house!.teleport(2.88, 5.9, 5.48))
  await walkTo(Math.PI, 'z', 7.2, false)
  await walkTo(Math.PI / 2, 'x', 1.6, true)
  await walkTo(Math.PI, 'z', 7.7, false)
  await page.evaluate(() => window.__house!.teleport(3.95, 0, 9.05))
