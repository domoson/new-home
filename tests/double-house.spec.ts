...
test('Mausrad zoomt 2D und steuert nur die avisierte Oeffnung', async ({ page }) => {
  ...
  for (const side of [1, -1]) {
    ...
    await expect(page.locator('#opening-select')).toHaveValue(`${side < 0 ? 'west-' : ''}EG-kitchen-window`)
    await expect(page.locator('#opening-amount')).toHaveValue('20')
    await page.locator('.scene-settings summary').click()
    await page.locator('canvas').hover(); await page.mouse.wheel(0, 100)
    await page.locator('.scene-settings summary').click(); await expect(page.locator('#opening-amount')).toHaveValue('10')
    await page.locator('.scene-settings summary').click()
...
test('Westhaelfte: gespiegelte Treppe und Kinderzimmer begehbar', async ({ page }, testInfo) => {
  ...
  await page.evaluate(() => window.__house!.teleport(-3.85, 2.95, 5.5))
  await walkTo(Math.PI/2, 'x', -5.3, true)
  await page.evaluate(() => window.__house!.teleport(-3.2, 2.95, 6.55))
  await walkTo(Math.PI, 'z', 8.35, false)
  expect((await page.evaluate(() => window.__house!.position())).y).toBeGreaterThan(3.8)
  await page.evaluate(() => window.__house!.teleport(-3.85, 2.95, 7.05))
  await walkTo(Math.PI / 2, 'x', -5.65, true)
})
...
test('Spiegelung, getrennte Farben, abschaltbare Umgebung und lichtdurchlaessiges Glas', async ({ page }, testInfo) => {
  ...
  await page.getByRole('button', {name:'Fassade Nebelblau', exact:true}).click(); await page.getByRole('button', {name:'Dach Naturrot', exact:true}).click(); await page.getByRole('button', {name:'Fensterrahmen Schwarz', exact:true}).click()
